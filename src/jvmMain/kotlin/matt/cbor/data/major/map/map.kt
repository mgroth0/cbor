...
class CborMap<K, V>(val value: Map<out CborDataItem<out K>, CborDataItem<out V>>) : SimpleStringableClass(),
    ...
    override fun toStringProps() = mapOf(
        ...
        if (raw.size <= 10) {
            mapOf(
                "Some Entries (need a better way to put hierarchical data in SimpleStringableClass strings)" to raw.map { (k, v) ->
                    "\n\t$k\t$v"
                }.joinToString("")
            )
